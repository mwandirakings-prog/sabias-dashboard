...
export default function SalespersonNotifications({ token, user }) {
  ...
  const generateNotifications = () => {
    ...
    if (myTotal > 0) {
      notifications.push({
        ...
        summary: `You have recorded MK ${fmt(myTotal)} across ${myTransactions} transactions.`,
  ...
  const getTypeStyle = (type) => {
    switch (type) {
      ...
      default:
        return { bg: '#E3F2FD', border: '#90CAF9',
    ...
    <div style={{ fontFamily: 'Arial' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)',
        ...
        {[
          ...
          { label: 'Sales', value: allNotifications.filter(
              n => n.category === 'Sales').length, color: '#2D6A4F' },
